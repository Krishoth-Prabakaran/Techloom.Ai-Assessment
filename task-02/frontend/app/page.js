...
import { ArrowRight, Check, Edit3, Moon, Plus, Search, ShoppingBag, Sun, Trash2, X } from 'lucide-react';
import './globals.css';
...
export default function Home() {
  ...
  useEffect(() => { setDark(localStorage.getItem('common-ground-theme') === 'dark'); request('/products').then(setProducts).catch((error) => setMessage(`Store connection unavailable: ${error.message}`)); request('/orders').then(setOrders).catch(() => {}); }, []);
  ...
  const checkout = async (outcome = 'success') => { try { const order = await request('/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ items: cart.map((item) => ({ productId: item.id, quantity: item.quantity })), sessionKey: crypto.randomUUID() }) }); const finalOrder = await request(`/orders/${order.id}/pay`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ outcome }) }); setOrders((items) => [finalOrder, ...items]); setCart([]); setMessage(`Order ${finalOrder.status}.`); await load(); } catch (error) { setMessage(`Checkout failed: ${error.message}`); } };
  ...
  return <><nav><a className="brand" href="/shop">COMMON<span>GROUND</span><small>OBJECTS FOR THE EVERYDAY</small></a><a className="nav-link active" href="/shop">Shop</a><a className="nav-link" href="/orders">Orders</a><button className="manage-trigger" onClick={() => setDrawer(true)}><Plus size={15} /> Curate</button><button className="theme-switch" onClick={() => setDark(!dark)} title="Toggle theme">{dark ? <Sun size={17} /> : <Moon size={17} />}<span>{dark ? 'Light' : 'Dark'}</span></button><button className="bag" onClick={() => document.getElementById('cart').showModal()}><ShoppingBag size={17} /><span>{cart.reduce((sum, item) => sum + item.quantity, 0)}</span></button></nav>
